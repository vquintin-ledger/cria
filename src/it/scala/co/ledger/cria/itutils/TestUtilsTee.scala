...
final class TestUtilsTee private (primary: TestUtils, secondary: TestUtils, combiner: Combiner)
  ...
  override def setupAccount(accountUid: AccountUid, walletUid: WalletUid): IO[Int] =
    combiner.combineAction(
      primary.setupAccount(accountUid, walletUid),
      secondary.setupAccount(accountUid, walletUid)
    )
...
object TestUtilsTee {
  def apply(primary: TestUtils, secondary: TestUtils, teeConfig: TeeConfig, log: IOLogger)(implicit
      lc: LogContext,
      cs: ContextShift[IO]
  ): TestUtils = {
