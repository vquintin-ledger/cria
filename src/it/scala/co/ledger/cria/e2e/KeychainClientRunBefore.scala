...
final class KeychainClientRunBefore(keychainClient: KeychainClient, action: IO[Unit]) extends KeychainClient {
  override def getLookaheadSize(keychainId: KeychainId): IO[Int] =
    action >> keychainClient.getLookaheadSize(keychainId)

  override def getAddresses(keychainId: KeychainId, fromIndex: Int, toIndex: Int, changeType: Option[ChangeType]): IO[List[AccountAddress]] =
  ...
  override def getKnownAndNewAddresses(keychainId: KeychainId, changeType: Option[ChangeType]): IO[List[AccountAddress]] =
