...
final class OperationServiceTee(
    primary: OperationService,
    secondary: OperationService,
    combiner: Combiner
) extends OperationService {
  override def deleteUnconfirmedOperations(accountId: AccountUid): IO[Int] =
    combiner.combineAction(
      primary.deleteUnconfirmedOperations(accountId),
      secondary.deleteUnconfirmedOperations(accountId)
    )
