...
class TransactionServiceTee(
    primary: TransactionService,
    secondary: TransactionService,
    combiner: Combiner
  ...
  override def deleteUnconfirmedTransaction(accountId: AccountUid, hash: TxHash): IO[String] =
    combiner.combineAction(
      primary.deleteUnconfirmedTransaction(accountId, hash),
      secondary.deleteUnconfirmedTransaction(accountId, hash)
    )
