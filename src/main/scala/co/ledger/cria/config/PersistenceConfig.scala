...
object PersistenceConfig extends PersistenceConfigLowPriority {
  ...
  final case class Both(primary: PersistenceConfig, secondary: PersistenceConfig, tee: TeeConfig) extends PersistenceConfig
  ...
  def foldM[M[_], A](wd: WalletDaemonDb => M[A], lama: LamaDb => M[A], both: (A, A, TeeConfig) => M[A])(c: PersistenceConfig)(implicit M: Monad[M]): M[A] =
    c match {
  ...
  def fold[A](wd: WalletDaemonDb => A, lama: LamaDb => A, both: (A, A, TeeConfig) => A)(c: PersistenceConfig): A =
    c match {
      case WalletDaemon(db) => wd(db)
      case Lama(db) => lama(db)
      case Both(c1, c2, tee) =>
